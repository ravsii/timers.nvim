...
describe("parse_format", function()
  ...
  it("throws on invalid character", function()
    assert.has_error(function()
      dur.parse_format("3x")
    end, "Unexpected char: x")
  end)
end)

describe("Duration:into_hms", function()
  it("formats durations less than 1 minute as seconds", function()
    local d = dur.from(45 * u.SECOND)
    assert.is_equal("45s", d:into_hms())
  end)

  it("formats durations between 1 minute and 1 hour as mm:ss", function()
    local d = dur.from(u.MINUTE + 15 * u.SECOND)
    assert.is_equal("01:15", d:into_hms())

    local d2 = dur.from(59 * u.MINUTE + 59 * u.SECOND)
    assert.is_equal("59:59", d2:into_hms())
  end)

  it("formats durations of 1 hour or more as hh:mm:ss", function()
    local d = dur.from(u.HOUR) -- exactly 1h
    assert.is_equal("01:00:00", d:into_hms())

    local d2 = dur.from(u.HOUR + u.MINUTE + u.SECOND) -- 1h 1m 1s
    assert.is_equal("01:01:01", d2:into_hms())

    local d3 = dur.from(2 * u.HOUR + 2 * u.MINUTE + 2 * u.SECOND) -- 2h 2m 2s
    assert.is_equal("02:02:02", d3:into_hms())
  end)

  it("handles zero duration", function()
    local d = dur.from(0)
    assert.is_equal("00s", d:into_hms())
  end)
end)
